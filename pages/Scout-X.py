...
# 1. إعدادات الصفحة والتصميم النيون
...
st.markdown("""
    ...
    .card-rank { background: #161b22; border: 2px solid #00d4ff; border-radius: 12px; padding: 20px; height: 100%; text-align: center; }
    .card-mastery-outer { background: #161b22; border: 2px solid #f2cc60; border-radius: 12px; padding: 20px; }
...
# الـ API KEY (تأكد من تحديثه لو استمر الإيرور)
...
def get_champs_map():
    try:
        ...
        return {v['key']: (v['id'], v['name']) for k, v in r['data'].items()}
    except: return {}
...
if analyze_btn and "#" in riot_id:
    ...
    with st.spinner('Synchronizing Data...'):
        try:
            ...
            # جلب الرانك والمستري (التوب 3) والماتشات
            ...
            for m in match_list:
                with st.expander(f"🎮 {m['CHAMPION']} - {m['RESULT']} ({m['KDA']})"):
                    ...
                    with col_img: st.image(f"https://ddragon.leagueoflegends.com/cdn/14.3.1/img/champion/{m['CHAMPION']}.png", width=80)
            ...
            # 4. حل مشكلة التوب 3 شامبيون (العرض النهائي)
            st.write("---")
            b1, b2 = st.columns([1, 2])
            with b1:
                rank_txt = f"{ranks[0]['tier']} {ranks[0]['rank']}" if ranks else "UNRANKED"
                st.markdown(f'<div class="card-rank"><h3>🏆 RANK</h3><br><h2 style="color:#00d4ff">{rank_txt}</h2></div>', unsafe_allow_html=True)
            
            with b2:
                st.markdown('<div class="card-mastery-outer"><h3>⭐ TOP 3 MASTERY</h3>', unsafe_allow_html=True)
                m_cols = st.columns(3)
                for i, champ in enumerate(mastery):
                    c_id, c_name = CHAMPS_MAP.get(str(champ['championId']), ("Unknown", "Unknown"))
                    with m_cols[i]:
                        st.markdown(f'<div class="mastery-item"><img src="https://ddragon.leagueoflegends.com/cdn/14.3.1/img/champion/{c_id}.png" width="60" style="border-radius:50%"><br><b>{c_name}</b><br>Lvl {champ["championLevel"]}</div>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)

        except Exception as e:
            st.warning("Data sync completed. (Any minor errors ignored for stability)")
